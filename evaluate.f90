...
type(item):: token(numtok)      ! List of tokens ( a token is an operator or 
                                ! operand) in postfix order
type(item) :: x,junk,tok
integer i,icp,insp,isum,ntok,ifound,j
real(kr8) value
...
! ----------------------------------------------------------------------
! Check the expression does not have too many tokens
! ----------------------------------------------------------------------

     tempstr=uppercase(adjustl(expr))
     call removesp(tempstr)
     ntok=0
     do
        call get_next_token(tempstr,tok,icp,insp)
...
!   print *,i,trim(x%char)
   ...
   if (x%type.eq.'S') then
      if (is_letter(x%char(1:1))) then

         ifound=0
         do j=1,nparams
!            print *,j,trim(params(j)%symbol)
            if (params(j)%symbol.eq.x%char) then
               x%ind=j
               ifound=1
               exit
            endif
         ...
         if (ifound.eq.0) then
            print *,'Tokenize_DC: could not find variable'
            return
         endif
         
         x%value=0
      else
!         print *,'x%char',trim(x%char)
         read (x%char,*) value
         x%value=value
         x%ind=0
      endif
...
subroutine evaltoken_DR(list,val)
  ...
  call evaltoken_DC(list,cval)

  val=real(cval,kind=kr8)
end subroutine evaltoken_DR

! ######################################################################

subroutine evaltoken_DC(list,val)
...
do i=1,ntok
   ...
   case('E')  ! Token is end token
      if(itop>1) then                
         ierr=12
         write(*,*) 'Error: missing operator in token list'
         write(*,*)
         return
      end if
      call popval(val)               ! Final result left on stack of values
   ...
   case('B')  ! Token is a binary operator
      if(itop < 2) then
         ierr=5
         write(*,*) 'Error in evaluation of token list'
         write(*,*) 'Less than two operands for binary operator  ',trim(x%char)
         write(*,*)
         return
      end if
      call popval(oper1)             ! Pull off top two values from stack
      ...
      case('/')
         if(oper1 == (0._kr8,0._kr8)) then
            ierr=10
            write(*,*) 'Error in expression (token list)'
            write(*,*) 'Division by zero'
            write(*,*)
            cval=0
         else
   ...
   case('F')  ! Token is a function name
      ...
      case('LN')
         if(real(oper1,kr8) <= 0. .and. aimag(oper1)==0.) then
            ierr=8
            write(*,*) 'Error: negative real or zero argument for',&
                 ' natural logarithm in expression'
            write(*,*)
            cval=0
         else
            cval=log(oper1)
